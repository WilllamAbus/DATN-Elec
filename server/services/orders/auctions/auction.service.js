...
const auctionService = {
  ...
  completeAuction : async (productId, timeTrackID) => {
    ...
    const bidEndTime = moment(timeTrack.endTime).tz('Asia/Ho_Chi_Minh');
    console.log('BidEndTime', bidEndTime);
    
  
    // Cập nhật stateBidding
    ...
    console.log('auction:', auctionTemp );
    ...
    if (moment(currentTime).isSameOrAfter(auctionEndTime)) {
      const biddings = await Bidding.find({ 'product_bidding.productId': productId }).sort({ bidAmount: -1 }).lean();
      ...
      console.log('highestBid: ', biddings);
      ...
      const updatedAuction = await Auction.findOneAndUpdate(
        ...
        { productId, status: 'active' },
        {
          $set: {
            ...
            auction_quantity: 1, // Cập nhật dựa trên logic của bạn
  ...
  getAll: async (page = 1, limit = 5) => {
    try {
      ...
      const totalAuctions = await Auction.countDocuments({ status: { $ne: 'disable' } });
  ...
  getById: async (auctionId) => {
    try {
      const auction = await Auction.findById(auctionId)
        .where('status').ne('disable').populate({
          path: 'auction_winner',
  ...
  softDelete: async (auctionId) => {
    try {
      const result = await Auction.findByIdAndUpdate(auctionId, { status: 'disable' }, { new: true });
  ...
  softDeleteList: async (query) => {
    try {
      const result = await Auction.updateMany(query, { $set: { status: 'disable' } });
  ...
  restore: async (auctionId) => {
    try {
      const result = await Auction.findByIdAndUpdate(auctionId, { status: 'active' }, { new: true });
  ...
  searchByWinnerName: async (name, page = 1, limit = 10) => {
    try {
      // Find users by name
      const users = await User.find({ name: new RegExp(name, 'i') }).select('_id').lean();
      if (users.length === 0) {
      ...
      const total = await Auction.countDocuments({ auction_winner: { $in: userIds } }); // Get total count for pagination
