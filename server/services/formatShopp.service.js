...
const formatService = {
  ...
  createFormat: async (formatData) => {
    ...
    if (existingFormat) {
      return res.status(400).json({ message: `The format "${formats}" already exists. Please use a different format.` });
    }
  ...
  deletedListFormat:async () => {
    try {
      // Validate page and pageSize parameters
      const deleteListFormat =  await _Format.find(   { status: 'disable' })|| {}; 
