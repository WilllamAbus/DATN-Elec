...
const prductFormatSchema = require("../product-v2.model"); // Import formatShoppingSchema từ mô hình của bạn

const orderCartSchema = new Schema(
  {
    user: { type: Schema.Types.ObjectId, ref: "users", required: true }, // Người dùng thực hiện thanh toán
    cartDetails: [{ type: Schema.Types.ObjectId, ref: "carts", default: [] }], // Tham chiếu đến giỏ hàng nếu có
    // Tham chiếu đến sản phẩm đấu giá thành công nếu có
    ...
    formatShipping: {
      // Thông tin về cách vận chuyển
      type: {
        type: String,
        enum: ["Tiêu chuẩn"], // Các loại vận chuyển
        required: true,
      },
      price: { type: Number, required: true }, // Phí vận chuyển tương ứng với loại
    },
    formatShopping: {
      // Thông tin về formatShopping
      type: prductFormatSchema, // Thay thế bằng kiểu của formatShoppingSchema
      required: true,
    },
...
module.exports = model("orderCarts", orderCartSchema);