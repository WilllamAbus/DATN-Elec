...
export const getOrders = createAsyncThunk<OrderResponse, { page: number; search?: string }, { rejectValue: string }>(
  'orders/getOrders',
  async ({ page, search }, { rejectWithValue }) => {
    const searchTerm = search ?? ""; // Fallback to an empty string if search is undefined

    try {
      const response = await getAllOrders(page, searchTerm);
      
      // Extract the orders array
   
      return response.data.data; // Return only the orders array
    } catch (error: any) {
