...
import { Order } from "../../../../types/adminOrder/getDeletedOrder";
...
const ListOrdersDelete: React.FC = () => {
  ...
  const deletedOrder = useSelector(
    (state: RootState) => state.getDeletedOrderAucAdmin.deletedOrders
  ) || []; // Default to an empty array if undefined
  const [, setOrder] = useState<Order[]>(deletedOrder);
  ...
  useEffect(() => {
    dispatch(getOrderDeletedThunk({ page: currentPage }));
  ...
  return (
    <>
      ...
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {Array.isArray(deletedOrder) && deletedOrder.length > 0 ? (
            deletedOrder.map((order: Order) => (
            ...
            <tr>
              <td colSpan={6} className="text-center py-4">No deleted orders found.</td>
            </tr>
          )}
        </tbody>
      </table>
