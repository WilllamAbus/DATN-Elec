...
const OrderListReceiveStatus: React.FC = () => {
  ...
  const orders = useSelector((state: RootState) => state.receiveStatusOrder.orderReceve);
  ...
  // Filter orders based on stateOrder === 'Nhận hàng'
  const filteredOrders = orders?.filter((order) => order.stateOrder === 'Nhận hàng');

  return (
    <div className="mt-6 border border-gray-300 pt-7 rounded-lg shadow-md bg-white">
      {filteredOrders?.map((order: OrderDataRecive, index: number) => (
